  ...
  def giveuserrole
   if current_user.superadmin_role? || current_user.supervisor_role? then
     ...
     if @user then
       ...
       flash[:notice] = "Роль Оператор назначена для #{@user.email}."
     else
  ...
  def givesupervisorrole
   if current_user.superadmin_role? || current_user.supervisor_role? then
     ...
     if @user then
       ...
       ActiveRecord::Base.connection.execute(sql)
       flash[:notice] = "Роль Администратор назначена для #{@user.email}."
     else
       flash[:warning] = "Нет пользователя с id=#{params[:id]}."  
     end
    redirect_to action: 'index'     
  ...
  def givenemorole
   if current_user.superadmin_role? || current_user.supervisor_role? then
     ...
     if @user then
       ...
       ActiveRecord::Base.connection.execute(sql)
       flash[:notice] = "Роль Немо назначена для #{@user.email}."
     else
       flash[:warning] = "Нет пользователя с id=#{params[:id]}."  
     end
    redirect_to action: 'index'     
 ...
 def dropallroles
   if current_user.superadmin_role? || current_user.supervisor_role? then
     ...
     if @user then
       ...
       ActiveRecord::Base.connection.execute(sql)
       flash[:notice] = "Все роли удалены для #{@user.email}."
     else
       flash[:warning] = "Нет пользователя с id=#{params[:id]}."  
     end
    redirect_to action: 'index'     
   ...
   if current_user.superadmin_role? || current_user.supervisor_role? then
     ...
     if user then
       ...
       if usercontracts==0 then 
        ...
        ActiveRecord::Base.connection.execute(sql)
        flash[:notice] = "Пользователь #{user.email} удален."
       else
        flash[:warning] = "Нельзя удалить пользователя #{user.email}, у которого #{usercontracts} контрактов! Просто сделайте его Немо или пошлите его к черту."
       end
     else
       flash[:warning] = "Нет пользователя с id=#{params[:id]}."  
     end
